...
# 쓰레드 : 싱글쓰레드 -> 멀티쓰레드 -> 복잡 -> 공유 되는 자원 -> 교착 상태 바라생 가능성, 컨텍스트 스위칭 비용이 발생, 자원 소비 가능성 증가
...
def coroutine2(x):
    ...
    y = yield x
    print(">>> coroutine recevied : {}".format(y))
    z = yield x + y
    print(">>> coroutine recevied : {}".format(z))
